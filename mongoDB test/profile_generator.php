...
function generateUsers()
{
    ...
    foreach ($cursor as $doc) {
        //var_dump($doc);
        $twoUsers = array();
        $data = array("collaborators" => array());
        $setArray = array();
        //get two random projects and store them in an array
        for($i = 0; $i < 5; $i++){
            $randomId = generateRandomUsers($database);
            array_push($twoUsers, $randomId);
        }
        $setArray = array("collaborators" => $twoUsers);
        $result = updateManyDB($database,"Users",$data, $setArray);
    }
    echo "DONE";
}
...
function generateRandomUsers($database){
    $result = randomDocument($database, "Users", "github", "https");
    foreach($result as $item){
        return getIdString($item["_id"]);
    }
}
